...
import { Ethereum, Solana } from "@thirdweb-dev/chains";
...
function WalletComponent() {
  ...
  useEffect(() => {
    const fetchEns = async () => {
      if (address && wallet?.ethers?.provider) {
        try {
          const ensName = await wallet.ethers.provider.lookupAddress(address);
          if (ensName) {
            setEnsName(ensName);
          }
        } catch (error) {
    ...
    <div className="wallet-section">
      <ConnectWallet
        ...
        welcomeScreen={{
          ...
          subtitle: "Connect with multiple wallet providers"
        }}
        modalTitleIconUrl="https://raw.githubusercontent.com/thirdweb-dev/js/main/packages/sdk/src/evm/constants/chain-icons/ethereum.svg"
        detailsBtn={() => {
          return (
...
function App() {
  ...
  const connectBackpack = async () => {
    try {
      ...
      try {
        ...
        const address = await window.backpack.sol.publicKey.toString();
        setWalletAddress(address);
        setIsConnected(true);
        setWalletProvider('Backpack');
        setShowWalletOptions(false);
        setEnsName(''); // Clear ENS as it's ETH specific

        // Listen for account changes
        window.backpack.on('accountChanged', handleBackpackAccountChanged);
      } catch (error) {
  ...
  // Handle Backpack account changes
  const handleBackpackAccountChanged = async () => {
    try {
      const address = await window.backpack.sol.publicKey.toString();
      setWalletAddress(address);
    } catch {
